...
- PrismaのLessonテーブルはバックオフィスからの公開設定・推定学習時間・レベル配列の整合性チェックに利用するため残し、JSONアセットと双方向で同期する
...
- 1回クリア=25%、4回のクリアで「マスター済み(100%)」扱いとなる。以降は100%を維持。
...
Future<void> submitLessonCompletion({
  ...
  Map<String, int> mistakeCharacters = const {},
}) async {
  final dio = Dio();
  ...
  try {
    final response = await dio.post(
      ...
      data: {
        ...
        'device': device,  // 'ios' | 'android' | 'web'
        'mode': mode,      // 'standard' | 'challenge'
        if (mistakeCharacters.isNotEmpty)
